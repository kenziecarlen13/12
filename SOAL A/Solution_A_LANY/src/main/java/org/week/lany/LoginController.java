...
public class LoginController {
    ...
    protected void btnLoginClick() throws IOException {
        ...
        if (inputUser.equals(ADMIN_USERNAME) && inputPass.equals(ADMIN_PASSWORD)) {
            LanySessionManager.getInstance().setCurrentUser(inputUser);
            LanySessionManager.getInstance().setUsername(inputUser);
            LanySessionManager.getInstance().setRole("Admin");

            alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setHeaderText("Information");
            alert.setContentText("Login success!! Welcome, Admin.");
            alert.showAndWait();
            Apps.setRoot("lany-inventory-view", "LANY Promotor Panel", false);

        // PATH 2: User Login (dari database)
        } else if (LanyDBConnection.validateUser(inputUser, inputPass)) {
            LanySessionManager.getInstance().setCurrentUser(inputUser);
            LanySessionManager.getInstance().setUsername(inputUser);
            LanySessionManager.getInstance().setRole("User");
